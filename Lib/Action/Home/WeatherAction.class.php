...
class WeatherAction extends BaseAction {
	...
	function shouldHandle($recvObj){
		...
		if (stripos($this->msg, "天气") !== false && $str1 != '翻译') {
			return true;
		}
		return false;
	}
	function handle(){
		$str_key = str_replace('天气', "", $this->msg);
		$data = $this->weather_search ( $str_key );
		...
		if ( $data->error != 0) {
			...
			$contentStr = "<item>\n
				<Title><![CDATA[【" . $data->results[0]->currentCity . "天气预报】]]></Title>\n
			...
			//数组循环转化
			foreach ( $weather_data as $value ) {
				$contentStr .= "<item>\n
				<Title><![CDATA[" . $value->date . "      " . $value->weather . "\n" . $value->wind . "　" . $value->temperature ."]]></Title>\n
				<Description><![CDATA[]]></Description>\n
				<PicUrl><![CDATA[" . $value->dayPictureUrl . "]]></PicUrl>\n
				<Url><![CDATA[]]></Url>\n
				</item>\n";
			}
  
			$count = count($weather_data) + 1;
			return parent::getNewsXml($count, $contentStr);
            //$contentStr = "【".$data->weatherinfo->city."天气预报】\n".$data->weatherinfo->date_y.$data->weatherinfo->fchh."时发布\n"."\n★实时天气"."\n气候：".$data->weatherinfo->weather1."\n气温：".$data->weatherinfo->temp1."\n风向风力：".$data->weatherinfo->wind1 . "\n紫外线强度：" . $data->weatherinfo->index_uv . "\n" . $text->Description . "\n温馨提示：".$data->weatherinfo->index_d."\n\n★明天天气"."\n气候：".$data->weatherinfo->weather2."\n气温：".$data->weatherinfo->temp2."\n风向风力：".$data->weatherinfo->wind1."\n\n★后天天气"."\n气候：".$data->weatherinfo->weather3."\n气温：".$data->weatherinfo->temp3."\n风向风力：".$data->weatherinfo->wind3;
